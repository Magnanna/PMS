import { db } from "@/db";
import { auditLogs, orgs } from "@/db/schema";
import { desc, eq } from "drizzle-orm";

export const dynamic = "force-dynamic";

/** US-J1/J3: cross-org superset of the per-org audit viewer, platform-staff only. */
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[var(--color-ink-500)] text-sm mt-1">Cross-org — last 300 events.</p>
      </div>
            ...
            <tbody className="divide-y divide-[var(--color-ink-100)]">
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="px-3 py-2.5 text-[var(--color-ink-400)]">{r.orgName ?? "—"}</td>
                  <td className="px-3 py-2.5 text-[var(--color-ink-400)] capitalize">{r.actorRole}</td>
